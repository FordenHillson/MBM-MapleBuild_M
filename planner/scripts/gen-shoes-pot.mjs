...
const inPath =
  'C:/Users/saetanpee/.cursor/projects/d-backup-perso-MSMTest-test/agent-tools/841463f9-a16b-4350-b830-740b7680bcf1.txt'
...
function addVal(data, rank, pool, optName, statStr) {
  ...
  if (!data[id]) {
    data[id] = {
      ...
      isPercent:
        id.endsWith('Percent') ||
        ['acc', 'spdPercent', 'expGainPercent'].includes(id),
...
function emitTs(exportName, ranksExport, options, comment) {
  ...
  out.push(`}`)
  out.push('')
  out.push(`/** Shoes ${comment} ranks — Nexon table 6444. */`)
  ...
  out.push(`]`)
  out.push('')
  out.push(`/** Shoes ${comment} — Nexon table 6444. */`)
...
fs.writeFileSync(
  path.join(outDir, 'potentialShoes.ts'),
  emitTs(
    'POTENTIAL_SHOES_OPTIONS',
    'POTENTIAL_SHOES_RANKS',
    mainOpts,
    'main Potential',
  ),
)
fs.writeFileSync(
  path.join(outDir, 'bonusPotentialShoes.ts'),
  emitTs(
    'BONUS_POTENTIAL_SHOES_OPTIONS',
    'BONUS_POTENTIAL_SHOES_RANKS',
    bonusOpts,
    'Bonus Potential',
  ),
)
