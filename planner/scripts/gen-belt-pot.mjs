...
const inPath =
  'C:/Users/saetanpee/.cursor/projects/d-backup-perso-MSMTest-test/agent-tools/d603c36a-9619-4d3d-9636-e9504e3ae34a.txt'
const lines = fs.readFileSync(inPath, 'utf8').split('\n')

const OPTION_MAP = {
  'PHY ATK Increase': 'phyAtkPercent',
  'MAG ATK Increase': 'magAtkPercent',
...
const LABELS = {
  ...
  phyDmgReducePercent: 'PHY DMG Reduction',
  magDmgReducePercent: 'MAG DMG Reduction',
  acc: 'ACC',
  maxHp: 'Max HP',
  maxMp: 'Max MP',
  maxHpPercent: 'Max HP %',
...
function emitTs(exportName, ranksExport, options, comment) {
  ...
  out.push(`}`)
  out.push('')
  out.push(`/** Belt ${comment} ranks — Nexon table 6451. */`)
  ...
  out.push(`]`)
  out.push('')
  out.push(`/** Belt ${comment} — Nexon table 6451. */`)
...
fs.writeFileSync(
  path.join(outDir, 'potentialBelt.ts'),
  emitTs(
    'POTENTIAL_BELT_OPTIONS',
    'POTENTIAL_BELT_RANKS',
    mainOpts,
    'main Potential',
  ),
)
fs.writeFileSync(
  path.join(outDir, 'bonusPotentialBelt.ts'),
  emitTs(
    'BONUS_POTENTIAL_BELT_OPTIONS',
    'BONUS_POTENTIAL_BELT_RANKS',
    bonusOpts,
    'Bonus Potential',
  ),
)
