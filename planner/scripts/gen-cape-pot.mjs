...
const inPath =
  'C:/Users/saetanpee/.cursor/projects/d-backup-perso-MSMTest-test/agent-tools/610b5153-b2e9-4ba8-89d7-a8e7dfe555d7.txt'
...
function emitTs(exportName, ranksExport, options, comment) {
  ...
  out.push(`}`)
  out.push('')
  out.push(`/** Cape ${comment} ranks — Nexon table 6452. */`)
  ...
  out.push(`]`)
  out.push('')
  out.push(`/** Cape ${comment} — Nexon table 6452. */`)
...
fs.writeFileSync(
  path.join(outDir, 'potentialCape.ts'),
  emitTs(
    'POTENTIAL_CAPE_OPTIONS',
    'POTENTIAL_CAPE_RANKS',
    mainOpts,
    'main Potential',
  ),
)
fs.writeFileSync(
  path.join(outDir, 'bonusPotentialCape.ts'),
  emitTs(
    'BONUS_POTENTIAL_CAPE_OPTIONS',
    'BONUS_POTENTIAL_CAPE_RANKS',
    bonusOpts,
    'Bonus Potential',
  ),
)
